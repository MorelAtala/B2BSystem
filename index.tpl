...
{block name="frontend_index_content_b2b"}
    ...
    <div class="panel b2b--main-panel">
        <div class="panel--body is--wide company-container" style="padding:0">
          ...
          <a href="{if {config name=always_select_payment}}{url controller='checkout' action='shippingPayment'}{else}{url controller='checkout' action='confirm'}{/if}"
             ...
             class="btn  btn--checkout-proceed is--primary right is--icon-right is--medium" >
              {s name="CheckoutActionsLinkProceedShort" namespace="frontend/checkout/actions"}{/s}
              <i class="icon--arrow-right"></i>
          </a>
            {*Ende Button Zur Kasse in Firma*}
  ...
  {* Automatische auswahl In der Firma*}
    <script>

    let element_to_open_id = 42;
    let action_to_do_id = "company-tab-panel";
  let checkExistence = setInterval(()=>{
      if($('li[data-id='+element_to_open_id+']').length > 0){
        $('li[data-id='+element_to_open_id+'] span.tree-label')[0].click()
        $('form[data-target='+action_to_do_id+'] button.is--b2b-acl-action-shipping').click()

        clearInterval(checkExistence)
      }
  }, 100)
    </script>
  {* Ende Automatische auswahl In der Firma *}
{/block}
