                ...
                <div class="panel panel--masterdata has--border is--rounded">
                    ...
                    <div class="panel--body">
                        <ul class="list--masterdata">
                              <li><span style="margin-right: 3rem;">{s name="CustomerNumber"}{/s}</span> {$identity->customernumber}</li>
                            {if $identity->debtor}
                                <li><span style="margin-right: 2rem;">{s name="AccountLevel"}Account level{/s}</span> {s name="Contact"}Contact{/s}</li>
                                <li><span style="margin-right: 2rem;">{s name="MyDebtor"}My debtor{/s}</span> {$identity->debtor->firstName} {$identity->debtor->lastName}</li>
                            {elseif $isSalesRep}
                                <li><span style="margin-right: 2rem;">{s name="AccountLevel"}Account level{/s}</span> {s name="SalesRepresentative"}Sales representative{/s}</li>
                            {else}
                                <li><span style="margin-right: 3rem;">{s name="AccountLevel"}Account level{/s}</span> {s name="Debtor"}Debtor{/s}</li>
                            {/if}

                            <li>
                                <span style="margin-right: 3rem;">{s name="Salutation"}Salutation{/s}</span>
                            ...
                            <li><span style="margin-right: 3rem;">{s name="Name"}Name{/s}</span> {$identity->firstName} {$identity->lastName}</li>
                            <li><span style="margin-right: 3rem;">{s name="Email"}E-Mail{/s}</span> {$identity->email}</li>
